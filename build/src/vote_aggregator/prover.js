import { Experimental, Circuit, MerkleMapWitness, Poseidon, Field, } from 'snarkyjs';
import { StateTransition, Vote } from './sequencer.js';
import { Nullifier } from './lib.js';
export { Prover };
function Prover(nullifierTree, voterData) {
    return Experimental.ZkProgram({
        publicInput: StateTransition,
        methods: {
            baseCase: {
                privateInputs: [Circuit.array(Vote, 3)],
                method(publicInput, votes) {
                    ...
                    for (let i = 0; i < 3; i++) {
                        ...
                        checkVoterEligibility(vote, voterData, publicInput).assertTrue('Voter is not an eligible voter!');
                        // making sure the voter actually voted for this proposal, preventing replay attacks
                        publicInput.proposalId.assertEquals(vote.proposalId, 'Vote proposalId does not match actual proposalId!');
                    ...
                    publicInput.result.after.yes.assertEquals(yes.add(publicInput.result.before.yes));
                    publicInput.result.after.no.assertEquals(no.add(publicInput.result.before.no));
                    publicInput.result.after.abstained.assertEquals(abstained.add(publicInput.result.before.abstained));
                    // we make sure that the final nullifier root is valid
                    tempRoot.assertEquals(publicInput.nullifier.after, 'Invalid state transition!');
                },
            },
        },
    });
}
function checkAndSetNullifier(vote, nullifierTree, nullifierRoot) {
...
function checkVoterEligibility(vote, voterData, publicInput) {
    ...
    let weight = Circuit.witness(Field, () => {
        return voterData.get(Poseidon.hash(vote.voter.toFields())).weight;
    });
    let [root] = membershipProof.computeRootAndKey(Poseidon.hash(vote.voter.toFields().concat(weight)));
